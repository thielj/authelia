...
func NewSessionWithAuthorizeRequest(ctx Context, issuer *url.URL, kid, username string, amr []string, extra map[string]any,
	authTime time.Time, consent *model.OAuth2ConsentSession, requester oauthelia2.AuthorizeRequester, claims *ClaimRequests) (session *Session) {
	if extra == nil {
		extra = map[string]any{}
	}
...
type Session struct {
	*openid.DefaultSession `json:"id_token"`
	...
	ClaimRequests         *ClaimRequests `json:"claim_requests,omitempty"`

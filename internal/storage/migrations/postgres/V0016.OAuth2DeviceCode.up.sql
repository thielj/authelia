...
CREATE TABLE IF NOT EXISTS oauth2_device_code_user_session (
    id SERIAL CONSTRAINT oauth2_device_code_user_session_pkey PRIMARY KEY,
    request_id VARCHAR(40) NOT NULL,
    client_id VARCHAR(255) NOT NULL,
    signature VARCHAR(255) NOT NULL,
    user_code_signature VARCHAR(255) NOT NULL,
...
CREATE INDEX oauth2_device_code_user_session_request_id_idx ON oauth2_device_code_user_session (request_id);
CREATE INDEX oauth2_device_code_user_session_client_id_idx ON oauth2_device_code_user_session (client_id);
CREATE INDEX oauth2_device_code_user_session_client_id_subject_idx ON oauth2_device_code_user_session (client_id, subject);

ALTER TABLE oauth2_device_code_user_session
    ADD CONSTRAINT oauth2_device_code_user_session_subject_fkey
        FOREIGN KEY (subject)
            REFERENCES user_opaque_identifier (identifier) ON UPDATE RESTRICT ON DELETE RESTRICT;

...
func NewSQLProvider(config *schema.Configuration, name, driverName, dataSourceName string) (provider SQLProvider) {
	...
	provider = SQLProvider{
		...
		sqlInsertOAuth2DeviceCodeUserSession:     fmt.Sprintf(queryFmtInsertOAuth2DeviceCodeSession, tableOAuth2DeviceCodeUserSession),
		sqlSelectOAuth2DeviceCodeUserSession:     fmt.Sprintf(queryFmtSelectOAuth2DeviceCodeUserSession, tableOAuth2DeviceCodeUserSession),
		sqlUpdateOAuth2DeviceCodeUserSession:     fmt.Sprintf(queryFmtUpdateOAuth2DeviceCodeUserSession, tableOAuth2DeviceCodeUserSession),
		sqlDeactivateOAuth2DeviceCodeUserSession: fmt.Sprintf(queryFmtDeactivateOAuth2DeviceCodeUserSession, tableOAuth2DeviceCodeUserSession),
...
type SQLProvider struct {
	...
	// Table: oauth2_device_code_user_session.
	sqlInsertOAuth2DeviceCodeUserSession     string
	sqlSelectOAuth2DeviceCodeUserSession     string
	sqlUpdateOAuth2DeviceCodeUserSession     string
	sqlDeactivateOAuth2DeviceCodeUserSession string
...
func (p *SQLProvider) SaveOAuth2DeviceCodeSession(ctx context.Context, session *model.OAuth2DeviceCodeSession) (err error) {
	...
	if err != nil {
		return fmt.Errorf("error inserting oauth2 device code session with signature '%s' for subject '%s' and request id '%s': %w", session.Signature, session.Subject.String, session.RequestID, err)
	}
...
func (p *SQLProvider) UpdateOAuth2DeviceCodeSession(ctx context.Context, signature string, status int, checked time.Time) (err error) {
	...
	if err != nil {
		return fmt.Errorf("error updating oauth2 device code session data with signature '%s': %w", signature, err)
	}
...
func (p *SQLProvider) SaveOAuth2DeviceCodeUserSession(ctx context.Context, session *model.OAuth2DeviceCodeSession) (err error) {
	if session.Session, err = p.encrypt(session.Session); err != nil {
		return fmt.Errorf("error encrypting oauth2 device code user session data for session with signature '%s' for subject '%s' and request id '%s': %w", session.Subject.String, session.Signature, session.RequestID, err)
	}

	_, err = p.db.ExecContext(ctx, p.sqlInsertOAuth2DeviceCodeUserSession,
		session.ChallengeID, session.RequestID, session.ClientID, session.Signature, session.UserCodeSignature,
		session.Status, session.Subject, session.RequestedAt, session.CheckedAt,
		session.RequestedScopes, session.GrantedScopes,
		session.RequestedAudience, session.GrantedAudience,
	...
	if err != nil {
		return fmt.Errorf("error inserting oauth2 device code user session data with signature '%s' for subject '%s' and request id '%s': %w", session.Signature, session.Subject.String, session.RequestID, err)
	}

	return nil
}

func (p *SQLProvider) UpdateOAuth2DeviceCodeUserSession(ctx context.Context, signature string, status int, checked time.Time) (err error) {
	_, err = p.db.ExecContext(ctx, p.sqlUpdateOAuth2DeviceCodeUserSession,
		checked, status, signature)

	if err != nil {
		return fmt.Errorf("error updating oauth2 device code user session data with signature '%s': %w", signature, err)
	}

	return nil
}

func (p *SQLProvider) DeactivateOAuth2DeviceCodeUserSession(ctx context.Context, signature string) (err error) {
	_, err = p.db.ExecContext(ctx, p.sqlDeactivateOAuth2DeviceCodeSession, signature)

	if err != nil {
		return fmt.Errorf("error deactivating oauth2 device code user session with signature '%s': %w", signature, err)
	}

	return nil
}

func (p *SQLProvider) LoadOAuth2DeviceCodeUserSession(ctx context.Context, signature string) (session *model.OAuth2DeviceCodeSession, err error) {
	session = &model.OAuth2DeviceCodeSession{}

	if err = p.db.GetContext(ctx, session, p.sqlSelectOAuth2DeviceCodeUserSession, signature); err != nil {
		return nil, fmt.Errorf("error selecting oauth2 device code user session with signature '%s': %w", signature, err)
	}

	if session.Session, err = p.decrypt(session.Session); err != nil {
		return nil, fmt.Errorf("error decrypting the oauth2 device code user session data with signature '%s' for subject '%s' and request id '%s': %w", signature, session.Subject.String, session.RequestID, err)
	}

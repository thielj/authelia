...
func TestShouldObtainCorrectMigrations(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			start := providerMigrationStart[tc.provider]
			...
			migrations, err = loadMigrations(tc.provider, "", 0, SchemaLatest)
			require.NoError(t, err)
			...
			if start != 1 {
				migrations, err = loadMigrations(tc.provider, "", 0, start-1)
				assert.EqualError(t, err, fmt.Sprintf("migrations between 0 (current) and %d (target) are invalid as the '%s' provider only has migrations starting at %d meaning the minimum target version when migrating up is %d", start-1, tc.provider, start, start))
			...
			if start > 1 {
				assert.EqualError(t, err, fmt.Sprintf("migrations between %d (current) and 1 (target) are invalid as the '%s' provider only has migrations starting at %d meaning the minimum target version when migrating down is %d with the exception of 0", ver, tc.provider, start, start))
...
func TestMigrationsShouldNotBeDuplicatedMSSQL(t *testing.T) {
	migrations, err := loadMigrations(providerMSSQL, "example", 0, SchemaLatest)
	require.NoError(t, err)
	require.NotEqual(t, 0, len(migrations))
	...
	migrations, err = loadMigrations(providerMSSQL, "example", SchemaLatest, 0)
	require.NoError(t, err)
	require.NotEqual(t, 0, len(migrations))
	...
	for i, migration := range migrations {
		...
		if migration.Query != "" {
			assert.Contains(t, migration.Query, "[example]")
		}

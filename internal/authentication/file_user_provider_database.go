...
type FileUserDatabaseUserDetailsAddressModel struct {
	...
	Country       string `yaml:"country" json:"country,omitempty" jsonschema:"title=Postal Code" jsonschema_description:"The country for the user."`
}
...
func (m FileDatabaseUserDetailsModel) ToDatabaseUserDetailsModel(username string) (model *FileUserDatabaseUserDetails, err error) {
	...
	if m.Website != "" {
		if model.Website, err = url.ParseRequestURI(m.Website); err != nil {
			return nil, fmt.Errorf("error occurred attempting to parse website: %w", err)
		}
	}

	if m.Profile != "" {
		if model.Profile, err = url.ParseRequestURI(m.Profile); err != nil {
			return nil, fmt.Errorf("error occurred attempting to parse profile: %w", err)
		}
	}

	if m.Picture != "" {
		if model.Picture, err = url.ParseRequestURI(m.Picture); err != nil {
			return nil, fmt.Errorf("error occurred attempting to parse picture: %w", err)
		}
	}
	...
	if m.Locale != "" {
		...
		if tag, err = language.Parse(m.Locale); err != nil {
			return nil, fmt.Errorf("error occurred attempting to parse locale: %w", err)
		}
